...
async def get_print_job_status(ip: str, port: int) -> PrinterStatus:
    response, print_job_info, temperature_info, status_info, printer_info = await collect_data(ip, port)
    if not response['online']:
        return response
    return parse_data(response, print_job_info, temperature_info, status_info, printer_info )
